...
1. Before editing, capture `node scripts/qa-runner.mjs session-start --session <task-id>`. This isolates task changes from user-owned dirty files. If work already began, keep an exact task file list and use `affected --files`.
2. Identify touched surfaces and likely blast radius.
3. Run `node scripts/qa-runner.mjs affected --session <task-id> --explain` (or `affected --files <paths> --explain`) and verify the impact selection before executing it.
4. Run the same command without `--explain`; this executes cheap preflight first, aggregates every failure in a phase, and uses content-keyed PASS cache locally.
5. Fix all reported failures from the phase together. Re-run only the exact failed gates and declared dependencies with `node scripts/qa-runner.mjs rerun-failed`; do not restart a group or the whole suite after each fix.
6. Run `node scripts/qa-runner.mjs full --no-cache` only for release certification, shared shell/service-worker changes, an explicit full-QA request, or when the scope matrix requires all browser shards.
7. Use `node scripts/qa-runner.mjs external --no-cache` only for deployed/provider claims. A local full PASS is not an external PASS.
8. Update QA/RULES/BUG docs when the result creates a recurring check, then report PASS/WARN/FAIL with the JSON report path and blocked checks.
...
| QA8 | Were failures fixed as one phase batch and rechecked with `rerun-failed` instead of repeatedly restarting full QA? |
| QA9 | Was impact derived from a task session or explicit task-owned files rather than the whole pre-existing dirty tree? |
