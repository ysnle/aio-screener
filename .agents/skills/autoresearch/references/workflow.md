...
For the repository skill system, infer stable representative prompts and negative-control claims from `architecture/skill-eval-cases.json`. Do not rewrite the task set between baseline and experiment.
...
- Record deterministic fixture validation (`ci-skill-eval-fixture-check.mjs`) and independent model execution as different result columns/statuses.
