...
- **v52.54 (2026-07-10, 배포 완료)**: WO-8 Packet 1(P669) — append-only 기록(CHANGELOG/BUG-POSTMORTEM 440+/600+ 항목)의 archive 분리는 대규모 콘텐츠 이동이라 범위 밖으로 이관, "INDEX/CODE-MAP 자동 현재성 검사"와 "감사 문서 resolved/open 명시"만 실행. 신규 `scripts/ci-doc-currency-check.mjs`(non-blocking) — `_context/CODE-MAP.md` §1 파일 크기 표가 v51.90(2026-07-02) 이후 무재검증으로 최대 484줄(aio-tests.js) 드리프트했음을 발견, CI 배선 후 §1을 실측치로 갱신(§2 이하는 미재검증 명시, confidence high→medium 정직 하향). 6개 주요 감사 문서 중 상태 배너 없던 유일한 문서(FABLE-LIVE-AUDIT-2026-07-04.md)에 resolved/closed-by-decision 요약 추가. 게이트: 로컬 8종(신규로 knowledge-lint까지 green) + 헤드리스 963/963 green.
- **v52.53 (2026-07-10, 배포 완료)**: WO-7 Packet 1(P668) — 자체 규칙("전체 재작성·대규모 전역 제거 금지", 여러 패킷에 걸친 점진적 작업)을 존중해 전역 read/write **인벤토리**(baseline 측정)만 수행. 실측: innerHTML 395·전역쓰기 1,318·이스케이프 호출 282·localStorage direct 146 vs safeLS 8. 핵심 발견: Codex가 "도입하라"던 timer/chart/page-lifecycle 어댑터는 **이미 존재**(`_aioTimerRegistry`/`_aioChartRegistry`/`_aioPageBus`) — 진짜 갭은 어댑터 부재가 아니라 채택률(이번 세션 반복 발견된 "인프라 있는데 미채택" 패턴). snapshot adapter만 진짜 부재. 위험 0에 가까운 수정 1건(raw setInterval→기존 레지스트리 경유)만 실행, storage adapter 전면화·snapshot adapter 신설·innerHTML 전수 분류는 별도 패킷으로 명시 이관(`_context/WO7-GLOBAL-INVENTORY-2026-07-10.md`). 게이트: 로컬 4종 확인+헤드리스 963/963 green.
- **v52.52 (2026-07-10, 배포 완료)**: WO-4 부분 구현(P667/R300) — 동시 실행 중인 별도 Codex 세션의 별개 문서(`CODEX-SECOND-PASS-HANDOFF-2026-07-10.md`, 사용자가 이번 지시 범위 밖이라 확인)에 이미 있던 진단(F-01/F-02)을 참고 정보로만 삼되 전부 직접 재검증(코드 미커밋 충돌 없음 확인 후) — `AIO_VIEWPORT_FULL_INIT=1`로 실행해 technical route 4콤보에서 `_pricePosition()` SVG 라벨-값 겹침을 재현·수정(baseline 간격 10px→14px). `pageerror`/`console.error` 수집을 viewport 게이트에 처음 추가 — 켜자마자 앱 자체의 정상적인 API 헬스 warn→error 로그 8건이 걸렸으나 소스 추적으로 버그 아님을 확인 후 정확한 패턴만 허용목록화(R300, 포괄 억제 아님). `zeroCanvases` 죽은 코드 실구현. small-text 정책 채택(8px 이하 실패, 9px 기존 34곳은 관찰만). 88/88 PASS 확인 후 `ci.yml` viewport-matrix를 `continue-on-error` 제거+FULL_INIT=1 기본+`deploy` needs 편입으로 실제 배포 차단 게이트 승격(R248과 동일 철학). 미충족: 외부 정상/timeout/partial-data 시나리오, 리소스 누수 계측, 키보드/screen reader 실사(자동화 불가) — DEFERRED-BLOCKS B9. 게이트: 로컬 7종 PASS+헤드리스 963/963 green.
- **v52.51 (2026-07-10, 배포 완료)**: WO-3 축소 검증(P666/R299) — WO-2와 동일한 벽(라이브 팩터 백테스트가 이미 873종목 1년치·6개 리밸런스만 사용) + WO-3 고유 신규 문제(오늘 유니버스 기준 다년치 fetch 시 survivorship bias, 무료 데이터로 해결 불가) 확인. 사용자 확인(AskUserQuestion, Yahoo IP 차단 이력 고려) 후 "제한된 표본"(시총 상위 120종목, concurrency=4)으로 진행 — `backtestFactors()`에 선택적 offsets/fwdDays 파라미터 추가(하위호환 확인) + `fetch-data.mjs`에 direct-run 가드 신설(부수 발견: 이 가드가 없어 다른 스크립트가 함수만 import해도 라이브 fetch 파이프라인 전체가 부작용으로 실행될 뻔했음) 후 신규 `scripts/backtest-factors-longrun.mjs`로 10년·117개 리밸런스(프로덕션 대비 ~20배) 검증. **결과**: composite는 1/5/21/63일 전 구간 통계적으로 무의미. lowvol 서브팩터만 5/21/63일에서 유의미한 음의 상관(21일 ICIR=-0.191/t=-2.04, 63일 ICIR=-0.308/t=-3.27, walk-forward holdout에서 더 강해짐) — 저변동성 팩터 통념과 반대 방향(표본 구성 차이가 유력 가설). survivorship bias는 여전히 미해결. 이 발견으로 라이브 `_aioComputeFactorRanks()` 코드 변경 없음(제품 결정 사항으로 보고). 게이트: 로컬 7종 PASS+헤드리스 963/963 green(런타임 무변경 확인).
- **v52.55~v52.61**: `_context/REMAINING-WORK-2026-07-12.md` 참조 (H2 잔여 항목/공개 게이트).
- **v52.62 (2026-07-12)**: `CLAUDE-CODE-HANDOFF.md`(아이보리 리디자인 시안 1b/2a/2b) P1~P2 전체 + P3~P10 부분 실행. **P1**: `:root` 전 토큰을 다크 네이비→웜 아이보리로 교체(색은 무채+상승green+하락red 2계열만), Inter/JetBrains Mono→Pretendard Variable/Noto Serif KR, 다크모드는 `light-theme`→`dark-theme` 클래스 롤네임+그래파이트 팔레트로 재정의. 4462행 부근 두 번째 `:root!important` 오버라이드 블록("v51.43 visual hierarchy refresh")이 소스 순서상 항상 이겨 토큰 교체를 무효화하던 함정을 발견해 함께 갱신(놓쳤으면 P1 전체가 시각적으로 무효였음). **P2**: badge/pill/버튼/CP1~8/nav-item 등 컴포넌트 규칙 적용 + 전역 하드코딩 색 스윕(rgba 945건+hex 190건+white-alpha 124건, 자동화 스크립트) + 이모지 221건 제거(방향 화살표·메뉴·새로고침 등 기능 글리프는 접근성상 보존) + 무지개 quality-meter 2건(고정배너/매크로 온도계) 단색화. **부수 발견 버그(P678/R309)**: 이모지 일괄 제거가 JS 문자열 내 조건부 체크마크 마커까지 지워 "바닥 확인 체크리스트"가 항상 "5/5 충족"을 반환할 뻔했고, close 버튼 5개가 accessible name 없이 비어버림 — 둘 다 발견 즉시 수정, R309로 재발방지 규칙화. **P3**(홈): 히트맵+GMO를 details로 접음. **P4**(시그널): CP1~8 details 접음(전체 섹션 재배열은 히든 DOM/테스트 의존 리스크로 보류). **P5~P10**: 전역 스윕은 전 페이지 반영, 페이지별 신규 와이어프레임(브리핑 2×2 그리드, 포트폴리오 미니차트, 차트 volume profile 등)은 JS 렌더러 재설계가 필요해 범위 밖 — 후속 세션 이관. 동시 진행 중이던 별도 Codex 세션(`c71587c` 등)과 index.html 겹치는 라인 없음 확인. 게이트: 로컬 9종+헤드리스 992/992 PASS+viewport-matrix/human-surface/portfolio-vault/a11y-matrix 확인.
- **v53.6~v53.7 (2026-07-17)**: **v53.6/P723·P724** — ticker 종목 개요 신설(밸리AI 참조: 좌 요약 레일+TradingView 대형 차트+수익률/테마 칩/팩터 레이더 서술형, KR 심볼 미지원 명시) + 잠재 버그 P724(Yahoo v7 quote 52주/거래량 확장 필드를 `_liveData`에 쓰는 코드 0곳 → applyLiveQuotes 보존 추가). **v53.7/P725** — 한국장 5페이지 통합(사용자 지시): kr-home/kr-supply 삭제, kr-themes/kr-macro/kr-technical을 themes/macro/technical의 접힌 "한국 시장" 섹션(`kr-integrated-*`)으로 이관(요소 id 보존), 라우트 22→17(REMOVED 5+canonical 리다이렉트), index.html -846줄. 게이트: 헤드리스 1101/1101+viewport 68/68+a11y 17+critical10 green, 실브라우저 리다이렉트/렌더 확인.
- **v52.68~v52.73 (2026-07-13)**: signal/briefing/breadth/sentiment/technical/macro/fxbond/themes/portfolio/market-news/screener 13개 시안 화면 전체 구조 재구축 완료(상세: CHANGELOG.md). 세션 중 사용자가 직접 지적한 반복 실수: "기존 구현이 시안보다 정교함"은 재구축을 건너뛸 이유가 아니었음 — fundamental(3f)/market-news(4b)/screener(4c) 3개를 헤더 폴리싱만 하고 "comp-compliant"로 잘못 기록했다가(P687) 재지적 받고 실제 구조 재구축(P688): fundamental엔 시안의 "기업개요 정성분석" 2열 섹션이 통째로 없었던 것을 발견해 신규 렌더 함수로 추가(FMP 실 데이터만 사용, 사업부문 매출비중처럼 없는 데이터는 미기재), market-news는 카드 우측을 topicBadge→sentWord로, screener는 컬럼 압축 토글 신규 추가. portfolio(4a) 잔여 구섹션 3종(AI운용노트/백테스트Lab/워치리스트+심화리스크)은 details로 통합, macro(3d)는 이미 이전 패스에서 완료돼 있던 것을 재확인. 게이트: 로컬 9종+헤드리스 992/992 PASS+viewport-matrix/critical10-human-surface/portfolio-vault-e2e/a11y-matrix 전부 확인.
