...
- **차기 작업 진입점**: `_context/FABLE-EFFICACY-AUDIT-2026-07-10.md` §5 — 라이브(v52.34) 페이지별 시장데이터 완비·사용성·실효성 실측 진단(EF-01~18: technical S&P 0.5% stale, breadth 4중 모순, 매크로 지표 1~2개월 stale, 엔캐리 게이지 상시 사망, ticker 재무 침묵 숨김, 어닝 캘린더 빈 화면 등) + 보강 설계 Batch 1~4(예정 v52.40~). 진단/설계는 Fable 5(코드 무변경), **구현은 Sonnet 5 세션이 §5 체크리스트대로**. 미점검 잔여(kr-technical·fundamental 실검색·AI 채팅 실문답 등)는 §4에 명시 — Batch 착수 전 보충 실측 필요.
