...
- **v52.1 Sonnet 5 Phase 3 [A1/B3] 페이로드 다이어트(P598)**: `public-data/telegram-digest.json`(부팅 시 동기 로드되는 단일 최대 페이로드, 2.4MB)에서 `items`(전체 병합셋 414건, ~1.04MB) 필드 제거 — 실사용처 전수 조사 결과 유일한 소비처(3순위 폴백)가 `broadItems`/`topItems` 둘 다 비어야만 도달하는데, `broadItems`(score≥50) 필터가 `topItems`(score≥65)보다 모든 기준에서 항상 더 느슨해 그 폴백이 수학적으로 도달 불가능함을 확인. 기존 P571/R262 주석이 이미 "items 미저장"을 명시했으나 실제 코드는 계속 포함시키던 것을 실제로 일치. **결과: 2.27MB→~1.22MB(약 46% 절감), 기능 변화 0**. **DATA_SNAPSHOT/CHAT_CONTEXTS 외부화는 검토 후 보류**: CHAT_CONTEXTS는 함수 값(`system: function(){...}`)이라 JSON화 자체가 불가능, DATA_SNAPSHOT은 237곳 직접 참조(SCREENER_DB의 144곳보다 많음, Phase 3 A3 매매점수 알고리즘도 포함)라 B6이 SCREENER_DB 비동기 전환을 보류한 것과 동일 이유로 보류. 격리 유닛테스트(실제 텔레그램 스크래핑 없음) + 로컬 validate 전체 + 헤드리스 2회 896/921(회귀 0). **Phase 3 [A1/B3] 완료**(요약/온디맨드 분리는 홈페이지를 포함한 12개 페이지가 전부 동일 풀에서 다른 태그로 필터링하는 구조라 보류 — 근거는 BUG-POSTMORTEM P598 참조). 남은 Phase 3: C3(매매점수 검증 하네스) → A4(상태 단일화), 각각 별도 계획 필요.
- **v52.0 Sonnet 5 Phase 3 [A2] 2단계 — defer 전환 완료(P597)**: 1단계(v51.99, P595/R275)에서 인라인 블록의 최상위 모듈 심볼 참조 22곳을 전부 방어한 뒤, `aio-core/data/ui/chat/glossary` 5개 스크립트에 `defer` 적용(로드맵 "5개 스크립트" 완료). 22페이지 순회 헤드리스 크롤러로 원본(A2 착수 전) 베이스라인과 최종 상태 직접 비교 — `CHAT_CONTEXTS` 20키·`_aioPageBus` 61등록 완전 동일, 헤드리스 2회 반복 896/921(회귀 0). **실측 성능**: 격리 git worktree로 A2 이전 커밋과 비교(Playwright 타이밍, 5회 재로드) — DOMContentLoaded는 거의 무변화(`<head>` preload가 이미 다운로드 병렬화, ~351ms→~344ms 예상대로 미미), **load 이벤트는 ~14.4초→~8.5초로 약 41% 단축**(세션 초반 예상보다 훨씬 큰 개선, 로컬 상대 비교치). **버전 v51.99→v52.0**(마이너 2자리 상한, v51.100 시도 시 ci-version-check.mjs 게이트 실패 실측). **Phase 3 [A2] 완료.** 남은 Phase 3: A1/B3(페이로드 다이어트) → C3(매매점수 검증 하네스) → A4(상태 단일화), 각각 별도 계획 필요.
- v51.99 이하 전체 버전 이력(Phase 0~3 로드맵 진행 상세 포함) → `CHANGELOG.md`
