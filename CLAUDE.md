...
- **v51.42 라이브 기본 경로 런타임 보강**: 배포된 v51.40 라이브 화면에서 `toFixed` 런타임 오류를 확인하고, 부분 라이브 데이터/홈 VIX/시나리오 합계/차트 툴팁/CPI 컨텍스트를 `window._aioSafeFixed` 기반으로 방어. 운영자 노트 최상단 승격은 유지하고 런타임 CI에 unsafe `toFixed` 회귀 게이트 추가. R1 7곳 v51.42.
