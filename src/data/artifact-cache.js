...
export function loadJsonArtifact(fetchFn, url, { signal, maxAgeMs = 15 * 60 * 1000, maxEntries = 64, timeoutMs = 15_000, force = false } = {}) {
  ...
  const fetchPromise = Promise.resolve()
    .then(() => fetchFn(key, { signal: controller.signal }))
    .then((response) => {
      if (!response?.ok) throw new Error(`Artifact ${response?.status || 'ERR'}: ${key}`);
