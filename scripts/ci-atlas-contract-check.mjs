...
if (data.telegramReferenceArtifact !== 'public-data/telegram-digest.json' || data.telegramReferenceChannels !== 4 || data.telegramObservedLineage !== 409 || telegramReference.collectionStatus !== 'failed' || telegramReference.channels.length !== 4 || telegramReference.sourceCatalog?.length !== 4 || telegramReference.channels.some((channel) => !channel.channel || channel.error === undefined)) errors.push('telegram discovery coverage');
...
const foundationSourceIdSet = new Set([...researchSourceIdSet, ...(foundationLessons.sourceCatalog || []).map((source) => source.id)]);
