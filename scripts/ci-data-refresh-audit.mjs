...
add('A3', 'VIX + HY OAS shared evidence', quote('^VIX')?.observedAt || macro._asOf_hyOAS, 'daily', `vix=${quote('^VIX')?.value ?? '—'} hyOAS=${macro.hyOAS ?? '—'} asOf=${macro._asOf_hyOAS || '—'}; session=${quote('^VIX')?.session || 'missing'}`, ageDays(macro._asOf_hyOAS) != null && ageDays(macro._asOf_hyOAS) > 2 ? 'SKIPPED' : null);
...
add('B4', 'Put/Call ratio', data.putCall?.asOf || data.meta?.putCallAsOf, 'daily', 'SKIPPED: durable Cboe artifact is 2026-07-24; no newer verified point was available and no value was synthesized', 'SKIPPED');
...
add('D1', 'HY OAS', macro._asOf_hyOAS || quote('HYG')?.observedAt, 'daily', 'SKIPPED: latest durable FRED observation is 2026-07-24; value retained as reference-only until a newer release is verified', 'SKIPPED');
...
add('E1', 'CPI / PCE', officialMacroFetchAt, 'monthly', `CPI=${macro.cpi ?? '—'} (obs ${macro._asOf_cpi || '—'}) PCE=${macro.pce ?? '—'} (obs ${macro._asOf_pce || '—'}) source=${macro._source || 'official runtime artifact'}`);
...
add('E3', 'FOMC calendar', '2026-07-16', 'monthly', 'manual official calendar reference; runtime result is not inferred from schedule');
add('E4', 'Central-bank policy rates', '2026-07-16', 'monthly', 'Fed/BOK manual official references are reference-only until point-in-time release reconciliation');
