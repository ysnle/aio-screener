...
async function main() {
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      ...
      const readBack = window._currentTickerId;
      const ctx = window.CHAT_CONTEXTS || {};
      const text = ctx.ticker && typeof ctx.ticker.system === 'function' ? ctx.ticker.system() : '(no fn)';
      return { readBack, sample: String(text).slice(0, 150), matches: /종목 cockpit|데이터 신뢰도/.test(text) };
    });
