...
const bundles = [
  ...
  ['lab', read('public-data/knowledge/quantitative-labs.json').labs || []]
];
const failures = [];
...
console.log(JSON.stringify({ status: failures.length ? 'FAIL' : 'PASS_WITH_BOUNDARY', bundles: bundles.map(([kind, items]) => ({ kind, count: items.length })), completionReady: false, failures }, null, 2));
