...
const checks = [
  ...
  { check: 'atlas taxonomy node coverage is 95 with connected relationships', pass: atlasIndex.taxonomyNodeCoverageArtifact === 'public-data/atlas/taxonomy-node-coverage.json' && atlasIndex.taxonomyNodeCoverage === 95 && atlasTaxonomyCoverage.nodes.length === 95 && atlasTaxonomyCoverage.coverage?.currentClaims === 0 && atlasTaxonomyCoverage.relationshipModel?.status === 'STRUCTURAL_RELATIONSHIPS_CONNECTED' && atlasTaxonomyCoverage.relationshipModel.domainChains.length === 19 && atlasTaxonomyCoverage.relationshipModel.domainChains.reduce((sum, chain) => sum + (chain.nodeIds?.length || 0), 0) === 95 && atlasTaxonomyCoverage.relationshipModel.crossDomainEdges.length === 18 },
  { check: 'telegram digest contains all 4 requested channels', pass: atlasIndex.telegramReferenceArtifact === 'public-data/telegram-digest.json' && atlasIndex.telegramReferenceChannels === 4 && telegramReference.channels.length === 4 && telegramReference.sourceCatalog?.length === 4 && (telegramReference.collectionStatus === 'failed' || (telegramReference.collectionStatus === 'ok' && telegramReference.successfulChannelCount === 4)) },
