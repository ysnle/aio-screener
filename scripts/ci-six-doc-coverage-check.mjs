...
const telegramReference = json('public-data/telegram-reference-window.json');
...
const checks = [
  ...
  { check: 'atlas authored foundation lessons are 48 and connected', pass: atlasIndex.foundationLessonArtifact === 'public-data/atlas/foundation-lessons.json' && atlasIndex.authoredFoundationLessons === 48 && foundationLessons.lessons.length === 48 && foundationLessons.shortFormStatus === 'AUTHORED_REFERENCE_CONNECTED' && foundationLessons.longFormStatus === 'AUTHORED_REFERENCE_CONNECTED' },
  ...
  { check: 'atlas taxonomy node coverage is 95 and current-claim safe', pass: atlasIndex.taxonomyNodeCoverageArtifact === 'public-data/atlas/taxonomy-node-coverage.json' && atlasIndex.taxonomyNodeCoverage === 95 && atlasTaxonomyCoverage.nodes.length === 95 && atlasTaxonomyCoverage.coverage?.currentClaims === 0 },
  { check: 'telegram reference window contains all 5 requested channels', pass: atlasIndex.telegramReferenceArtifact === 'public-data/telegram-reference-window.json' && atlasIndex.telegramReferenceChannels === 5 && telegramReference.channels.length === 5 && telegramReference.promotedClaims === 0 },
  ...
  { check: 'atlas player/product reference registry is 20/20/20', pass: atlasIndex.referencePlayers === 20 && atlasIndex.referenceProducts === 20 && atlasIndex.referenceSources === 20 && playerProduct.players.length === 20 && playerProduct.products.length === 20 && playerProduct.sources.length === 20 },
  { check: 'atlas player/product currentness overlay is 20/20', pass: atlasIndex.playerProductCurrentnessArtifact === 'public-data/atlas/player-product-currentness.json' && playerProductCurrentness.players.length === 20 && playerProductCurrentness.products.length === 20 && playerProductCurrentness.products.every((item) => item.productionStatus && item.asOf) },
  ...
  { check: 'principles A~O lesson library is 111 and fully sourced', pass: principlesLessonLibrary.status === 'REFERENCE_CONNECTED' && principlesLessonLibrary.lessons.length === 111 && principlesLessonLibrary.lessons.every((lesson) => lesson.sourceIds?.length && lesson.diagram && lesson.verificationQuestion) },
  { check: 'masters full holdings count is 1248', pass: mastersHoldings.allHoldings?.length === 1248 },
  { check: 'masters full comparison count is 1377', pass: mastersHoldings.comparisons?.length === 1377 },
  { check: 'masters latest available period is 2026-03-31', pass: mastersFilings.latestAvailablePeriod === '2026-03-31' },
  ...
  { check: 'masters security master artifact is connected fail-closed', pass: mastersSecurityMaster.status === 'REFERENCE_NORMALIZATION_PENDING' && mastersSecurityMaster.coverage?.rawUniqueCusips === 1102 && mastersSecurityMaster.coverage?.mappedRows === 0 && mastersSecurityMaster.records?.length === 0 }
...
const incomplete = [
  'Industry Atlas primary evidence ledger for every current numeric, product-generation and production-state claim; structural reference ledger is connected',
...
console.log(JSON.stringify({
  ...
  strict,
  observed: { principlesNodes, principlesLessons, principlesPaths, principlesEdges, principlesAuthoredChapters: principlesChapters.chapters.length, principlesLessonLibrary: principlesLessonLibrary.lessons.length, atlasPrimarySources: research.sources.length, atlasEvidenceClaims: research.claims.length, atlasCandidateEdges: research.edges.length, atlasFoundationLessons: foundationLessons.lessons.length, atlasDomainGuides: atlasDomainGuides.guides.length, atlasDomainPackets: atlasDomainPackets.packets.length, atlasDomainStructuralClaims: atlasDomainClaims.claims.length, atlasTaxonomyCoverage: atlasTaxonomyCoverage.nodes.length, telegramChannels: telegramReference.channels.length, telegramObservedLineage: telegramReference.observedLineageCount, atlasReferencePlayers: playerProduct.players.length, atlasReferenceProducts: playerProduct.products.length, atlasReferenceSources: playerProduct.sources.length, atlasCurrentnessProducts: playerProductCurrentness.products.length, mastersSecurityMasterStatus: mastersSecurityMaster.status, mastersSecurityMasterRecords: mastersSecurityMaster.records.length, mastersReferenceMappings: mastersReferenceMaster.records.length, mastersHistoryManagers: mastersHistoryIndex.connectedManagers, mastersHistoryPeriods: mastersHistoryIndex.totalPeriods, mastersHistoryRows: mastersHistoryRows.rowsImported },
