...
const checks = [
  ...
  ...['definition', 'mechanism', 'example', 'counterScenario', 'verificationQuestion', 'diagram'].map((field) => ({ check: `principles ${field} text is unique per lesson`, pass: new Set(principlesLessonLibrary.lessons.map((lesson) => lesson[field])).size === 111 })),
  { check: 'masters full holdings count is 1248', pass: mastersHoldings.allHoldings?.length === 1248 },
  { check: 'masters full comparison count is 1375', pass: mastersHoldings.comparisons?.length === 1375 },
