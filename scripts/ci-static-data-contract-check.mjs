...
const prohibited = [
  ...
  [/시총 ~[\d.]+조/, 'point-in-time market-cap annotation in static theme map (R604)'],
  ...
  [/AIO_EVENT_FRESHNESS_REGISTRY\s*=\s*\{(?![\s\S]{0,3000}?maxClaimAgeDays\s*:)[\s\S]{0,2000}?(result|marketReaction)\s*:/, 'event narrative without expiry (P1081: registry entries must carry eventDate + maxClaimAgeDays so getEventClaimState can expire them)'],
