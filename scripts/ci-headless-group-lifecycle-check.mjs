...
if (!headless.includes("value.startsWith('--groups=')") || !headless.includes('AIO_FAILED_GROUPS=') || !/runResult\?\.selection\?\.mode|result\?\.selection\?\.mode/.test(headless)) fail('headless runner lacks exact group selection/failure output');
if (!headless.includes('writeQaReport') || !headless.includes('failure report=') || !headless.includes('groupResults')) fail('headless runner lacks durable failure details/report artifact output');
