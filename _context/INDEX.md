...
| `MARKET-PRINCIPLES-ATLAS-STRUCTURAL-AUDIT-HANDOFF-2026-08-10.md` | v53.99 저장소·artifact·라이브/로컬 UI 전수 대조 SSOT. Principles 60/71/111과 AI 시대 지식 지도 48/19/95/50의 의미·연결·출처·학습 상태, 159개 원고의 백과사전 깊이, 9개 사용자 관점과 KA-00~10 구조 개편 패킷을 고정. KA-00 VERIFIED_LOCAL, KA-01/02/03/08 부분 기준 구현, 심층 원고·학습 상태·전문 route·실사용자 연구는 미완료 | KA 패킷 상태, knowledge ontology/schema, graph component/edge, source resolver, lesson/article depth, persona journey, route state, renderer 또는 라이브 판정 변경 시 |
| `MARKET-PRINCIPLES-ATLAS-AUDIT-CONTRACT-2026-08-10.json` | 위 구조 개편 핸드오프의 기계 판독 계약. v53.98 기준선과 v53.99 로컬 교정 상태, 백과사전 blocker, KA-00~10 의존·구현 상태, reference-core 파일, persona와 필수 semantic gate를 고정 | baseline/current 수치, blocker ID/status, depth/persona contract, work-package dependency/status, required gate 또는 handoff 상태 변경 시 |
| `MARKET-PRINCIPLES-ATLAS-HANDOFF-FILE-MANIFEST-2026-08-10.json` | 구조 개편 핸드오프 패키지의 15개 파일·역할·시작 순서·금지 완료 주장을 고정하는 전달용 manifest | 패키지 파일, entry point, 실행 순서, reference implementation 또는 completion boundary 변경 시 |
