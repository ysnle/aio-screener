...
|---|---|---|---|
| 미국 시세/일봉 | Yahoo + Twelve/Finnhub 폴백 | Massive 또는 Alpaca의 라이선스된 HTTP/WebSocket/벌크 경로 | 대체 가능, 플랜·표시권 검토 필요 |
| 미국 재무 | FMP 플랜 의존, SEC 부분 경로 | SEC companyfacts/frames/bulk ZIP + 자체 XBRL 정규화; Intrinio 유료 보조 | 공식 무료 원천 가능, 비교 가능한 TTM 정규화는 구현 비용 큼 |
...
| Put/Call | Cboe 페이지/CDN + 스냅샷 | Cboe 공식 일별/현재 통계 ingest 또는 옵션 벤더 | 자동화 가능, HTML/CDN 변경과 권리 점검 필요 |
| AAII/NAAIM/II | 주간 스냅샷 | 공식 라이선스/구독 또는 수동 검증 | 무료 동등 API 없음. VIX/F&G로 바꾸면 다른 지표임 |
| 뉴스/이벤트 | 다중 RSS + 선택적 Finnhub/NewsData | GDELT Event/GKG + 규제기관/기업 RSS + 유료 뉴스 | 대체·중복화 가능, 본문 재배포 권리 분리 필요 |
| Telegram | 공개 웹/RSS mirror | 관리 채널은 Bot API; 임의 공개 채널은 승인된 MTProto/TDLib 세션 또는 라이선스 집계 | 익명 스크래핑은 운영 주경로로 부적합 |
| 옵션 체인/Greeks | FMP 등 선택 공급자 | Massive/Intrinio/Alpaca/Cboe 계열 상품 | OPRA/거래소 권리와 유료 플랜 필요 |
| 어닝/경제 캘린더 | Finnhub → FMP | 벤더 캘린더 + SEC/기업 IR/공식 발표 일정 교차 | 자동화 가능, 단일 무료 소스로 완전 대체 어려움 |
...
2. **다음 P0**: GitHub Actions에서 `SCREENER_ONLY`를 별도 job으로 분리하고 산출물 semantic 검증 후 원자적 publish.
3. **다음 P1**: SEC/OpenDART 정규화 어댑터와 수정공시/기업행위 저장소.
4. **다음 P1**: KRX Open API 승인·표시·재배포 조건 확정 후 EOD 경로 교체.
5. **다음 P1**: Cboe Put/Call 공식 ingest와 설문 데이터의 라이선스/수동 상태 분리.
6. **다음 P2**: 라이선스된 미국 시세/옵션 공급자를 하나 선택해 Yahoo를 보조 경로로 강등.
...
- Massive stocks API: https://massive.com/docs/rest/stocks
