...
- Do this instead of either (a) leaving the failure unretried and letting the next scheduled cycle happen to fix it, or (b) pulling in a marketplace retry-wrapper action for a need two plain conditional steps already cover.
- A silent, unretried failure in a deploy step is not contained to "one CI run failed" if something else independently observes the *consequence* of that failure — a separate freshness/health-check workflow (e.g. `data-watchdog.yml`) will also fail once the resulting staleness crosses its own threshold, producing a second, seemingly-unrelated failure notification for the same root cause, hours later. When investigating "why do I keep getting failure emails" from more than one workflow, check whether they share a causal chain before treating them as separate problems.
- See P601/BUG-POSTMORTEM.md: `actions/deploy-pages` failed with `Deployment failed, try again later.` on 4 of 16 (25%) `workflow_run`-triggered deploys in the first ~33h after R272/P591 wired up the bot-commit-to-deploy path — each one separately tripped `data-watchdog.yml`'s live-site-freshness check hours later, and each of the two workflows' failures independently emails on GitHub's default per-run notification.
