...
- **fix**: 구형 KV 핸들러를 제거해 단일 원자적 핸들러로 축약하고, Durable Object migration/binding 및 Anthropic 허용 지역 `aws:us-east-1` placement를 가진 `worker/wrangler.proxy.toml`을 추가한다. `deploy-ai-proxy.yml`은 시크릿 요구·배포·health/readiness·CORS·거부 origin뿐 아니라 실제 최소 Anthropic upstream 응답까지 검사한다. operations producer는 관측된 proxy evidence로 publicChat 상태를 계산한다.
