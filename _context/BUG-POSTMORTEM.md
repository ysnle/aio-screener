...
## P638 · v52.24 · Deployed Cloudflare Worker predates the repo's `/anthropic` server-key route — root-caused, not code-fixable this session (operator action required)
...
- **root_cause**: Confirmed by reading the full `cloudflare-worker-proxy.js` end to end: the repo's current script correctly routes `pathname === '/anthropic'` to `handleAnthropic()` *before* the generic GET-only branch (added v50.52/B5), which would answer a POST with `'POST required for /anthropic'` (a different message) if it were live. The actual observed error text ("GET 요청만 지원됩니다") is only reachable from the *generic proxy's* GET-only guard, which is unreachable in the current script once the `/anthropic` pathname check exists — meaning the **deployed** Worker at `aio-proxy.zmfhd007.workers.dev` is running an older revision that predates the v50.52 `/anthropic` route entirely (matching the same file's own now-stale header comment: "AI 채팅 Claude 키는 ... 이 Worker 경유 아님"). This exactly matches the already-tracked `_context/DEFERRED-BLOCKS.md` **B5** entry ("Claude 키 서버화 — 운영자 결정 + Worker secret 배포 필요") — this is not a new bug, it is B5's known blocked state surfacing as a 405 in production.
- **fix**: None applied — deploying a Cloudflare Worker requires dashboard/`wrangler` credentials this session does not have. No code change can close this; `cloudflare-worker-proxy.js` in the repo is already correct.
- **violated_rule**: None — this is an infrastructure/operator gap, not a rule violation.
- **prevention**: Operator action needed: (1) redeploy the current `cloudflare-worker-proxy.js` to the live Worker (dashboard copy-paste or `wrangler deploy`), (2) add the `ANTHROPIC_API_KEY` secret if enabling server-key mode for all users. Until then, P637's Google-Translate cascade keeps translation degraded-but-functional; AI briefing/ticker-AI generation has no equivalent client-side fallback and will keep failing visibly.
- **verification**: N/A (no code change). Confirmed by full read of `cloudflare-worker-proxy.js` (lines 173-245) and cross-reference with `_context/DEFERRED-BLOCKS.md` B5 and `_context/OPUS-HANDOFF-STRUCTURAL-AUDIT-2026-06-10.md` WO-10.
