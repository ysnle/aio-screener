...
confidence: high
latest_version: v48.13
latest_P_number: P128
next_P_number: P129
total_entries: 128
---
...
| **P128** | **v48.13** | **2026-04-17** | **기업 분석 기업뉴스 섹션 부재 + 포트폴리오 벤치마크 차트 stub 버그**. (1) v47.10에서 fetchFinnhubCompanyNews dead code 제거 후 재도입 안 됨. 기업 분석 페이지에 최근 기업 뉴스 전용 섹션 없어서 사용자가 개별 기업 소식을 별도로 검색해야 했음. (2) updateBenchmarkChart(L40809)는 DOM canvas pf-benchmark-chart를 대상으로 하지만 포트폴리오 필드명 p.sym/p.avgCost/p.shares 오표기(getPortfolioData 실제 필드는 ticker/cost/qty) → totalInvested 항상 0 → pfReturn 항상 0 표시. SPY 수익률도 DATA_SNAPSHOT.spxPct 단일값을 30일 선형 보간한 stub. 포트폴리오 대시보드 핵심 지표 무효화. **수정 2건**: (a) fetchFinnhubCompanyNews(symbol, daysBack=14) 재도입 — 최신순 정렬 + headline 중복 제거 + 상위 15건 반환. fundamentalSearch의 Finnhub Promise.allSettled에 4번째 job으로 편입 → collected.finnhubNews 저장 + sources 'Finnhub (기업 뉴스)'. _renderFundNews(d) 신설 — 헤드라인 140자 + 요약 180자 + N시간/일 전 + source/date 라벨. fund-rpt-news DOM은 fund-rpt-earnings 뒤·fund-rpt-sources 앞 자연 삽입. XSS escHtml 전수. 기존 _renderFund* 패턴 100% 준수. (b) updateBenchmarkChart 필드명 교정(ticker/cost/qty) + SPY Yahoo chart range=1mo 30일 실데이터 fetch + 포트폴리오 상위 10 종목 Promise.all 병렬 fetch → 가중 일별 수익률. 미커버 종목은 현재 누적 수익률 선형 분포 폴백. 0% 기준선 dashed, 공통 팔레트 색상(#60a5fa SPY / #3ddba5 PF / #f87171 마이너스 Alpha), 커버리지 % 하단 표시. 빈 포트폴리오 안내 메시지. **예방**: (1) dead code 제거 시 '향후 재도입 고려'로 표기한 함수는 분기별 재검토 — 기능 공백 자동 트래킹. (2) canvas/DOM 대상 함수는 참조 필드명이 실제 데이터 스키마와 일치하는지 쌍대 grep 검증(P43 stale DOM 패턴의 변형). (3) 'stub 함수' 태그를 명시(// STUB) 하여 실데이터 연결 누락 추적. violated_rule: R15(데이터 미수신 vs 0% 구분) + P43(stale reference 변형) |
| **P127** | **v48.12** | **2026-04-17** | **AI 채팅 기관 리서치 스타일 부재 + Finnhub recommendation 부분 주입 + 뉴스 desc 누락**. 사용자 보고: 최근 주식/기업 소식·기관 애널리스트 등급·논리적 이유·인사이트 핵심을 AI 답변에 반영 필요. 실측 결과 3가지 누락: (1) Finnhub recommendation + /calendar/earnings는 fundamentalSearch(기업 분석 페이지 검색) 경로에서만 collected.finnhubRecommendation/finnhubEarnings 주입, 일반 AI 채팅 _fetchTickerDataForChat은 누락 — 사용자가 AI 패널에서 "NVDA 어때?" 질문해도 기관 컨센서스 미반영. (2) _buildNewsContext가 제목 80자만 주입 — 뉴스 본문 desc/summary 미포함으로 '왜 올랐나/내렸나' 맥락 부족, 헤드라인만으로 AI가 근거 부족한 답변. (3) 시스템 프롬프트에 'Bull Case/Base Case/Bear Case + 카탈리스트 + 해자 + 깨지는 신호' 구조화 지시 없음 — AI가 단편적 의견("좋다/나쁘다") 답변, 기관 리서치 수준 논리 근거 부족. **수정 3건**: (a) _fetchTickerDataForChat 루프에서 각 티커마다 fetchFinnhubRecommendation + fetchFinnhubEarningsCalendar(today~+60d) 자동 호출 + '[애널리스트 컨센서스] N명 · StrongBuy/Buy/Hold/Sell/StrongSell → 매수 우세/완만/중립/매도 우세' + '[다음 어닝] 날짜·장전/장후·예상 EPS·예상 매출' 주입. (b) _buildNewsContext 각 뉴스 항목에 desc/summary/description 필드 HTML 스트립 후 140자 요약 + '└' 인덴트 추가. 헤드라인 80→100자 확장. 지시문도 '요약의 사실 근거(숫자·기관명·인용) 적극 활용'으로 변경. (c) chatSendUnified/chatSend 양쪽에 기관 애널리스트 스타일 답변 구조 6단계 지시 — 골드만·JP모건·버핏·ARK 레퍼런스, ①현재 상황 ②투자 스토리/내러티브(해자 5유형·성장 동력·테마) ③재무·밸류에이션('왜 그 수준인가') ④Bull Case(목표가 + 달성 조건) ⑤Bear Case(하락 트리거) ⑥카탈리스트·리스크·깨지는 신호 3~5개. 모든 주입 데이터 교차 참조 + 숫자·시점·출처 근거 + 인사이트 핵심 먼저 명시 + '카더라/일반적으로' 두루뭉실 금지. **예방**: (1) LLM 통합 시 동일 데이터를 '수집 경로 A / 경로 B'로 이원화하면 누락 발생 — fundamentalSearch와 _fetchTickerDataForChat 같은 공용 fetcher에서 모든 외부 API 호출을 중앙화. (2) 뉴스 주입 시 헤드라인만은 '진짜 이유'를 놓침 — desc/content 요약 120~150자 포함 표준화. (3) AI 답변 프레임워크는 도메인 특화 구조(Bull/Base/Bear·해자·카탈리스트) 명시 필요 — 단순 '논리적으로 답하라' 지시만으론 부족. violated_rule: 신규(fetcher 이원화 누락) + R29(AI 검증 태그 연장) |
| **P126** | **v48.11** | **2026-04-17** | **AI 채팅 추세 전환 미반영 + 오늘 날짜·커트오프 부재 + Perplexity recency 고정**. 사용자 보고: '이전 긍정 뉴스로 상승했던 종목이 최근 하락으로 전환됐는데 AI는 여전히 추천'하는 시간 불일치 케이스. 실측 결과 5가지 사각지대: (1) _fetchTickerDataForChat이 당일 변동만 주입, 5D/20D/3M 주가 추이 없음 — AI가 최근 추세 전환 감지 불가. (2) chatSendUnified/chatSend system prompt에 '오늘 날짜' 명시 부재 — Claude가 학습 시점 기준 '최근'을 사용자 현재 기준 '최근'으로 착각 가능. (3) Claude 학습 커트오프(2025년 초) 경고 없음 — 기억 속 구형 주가/뉴스로 답변하는 근본 방지 장치 부재. (4) 추세 해석 지시 부재 — '긍정 뉴스 + 주가 하락 = 재료 소진' 같은 시간 불일치 해석 규칙 없음. (5) Perplexity search_recency_filter 'week' 고정 — '오늘 주가' 같은 당일 질문에도 1주일 결과 반환. **수정 5건**: (a) _fetchTickerTrend(ticker) 신설 — Yahoo chart range=3mo 1회 호출로 5D/20D/3M 변동 + 추세 라벨(단기·중기 상승/하락/반등 초기/조정 중/횡보 등 8종) + 3M 범위 위치 %. _tickerTrendCache 10분 TTL. _fetchTickerDataForChat 루프에서 '[주가 추이]' 라인 자동 주입. (b) '오늘 날짜 (KST) + 학습 커트오프 2025년 초' 경고 블록 system prompt 상단 삽입 — Claude가 네 기억 속 '최근'이 사용자 기준 과거임을 의식. (c) 추세 해석 필수 규칙 6개 — 긍정 뉴스+하락 추세 시 뉴스만 추천 금지, 반드시 [주가 추이] 라벨 먼저 확인, 애널리스트 목표가 발표 시점 확인, 시간 불일치 탐지로 재료 소진/후속 악재 언급. (d) 데이터 검증 태그 강화: _quoteTimestamps 분 단위 stale 체크(5분+ 주의/10분+ 지연 명시/미수신 시 가격 인용 금지), [주가 추이] 주입 여부 별도 항목, DATA_SNAPSHOT 72h+ 경과 시 정적 수치 인용 자체 금지. (e) Perplexity search_recency_filter 쿼리 키워드 동적 — '오늘/지금/금일/방금/현재/당일/today/now' 포함 시 'day' 그 외 'week'. chatSendUnified + chatSend 양쪽 일관 적용. **예방**: (1) LLM 통합 시 '오늘 날짜 + 모델 커트오프' 주입은 반드시 필수 (Claude 포함 모든 LLM). (2) 주가/뉴스 같은 시계열 데이터는 '현재 시점 + 추이 + 시간 가중치' 3요소를 시스템 프롬프트에 주입해야 temporal awareness 확보. (3) 검색 API recency는 쿼리 키워드 기반 동적 적용 — 'week' 기본값 + '오늘' 언급 시 'day'. violated_rule: R29(AI 채팅 데이터 검증 태그) 연장 + 신규(temporal awareness 부재) |
