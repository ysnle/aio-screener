...
9. ✅ **[D-1/C2] 백테스트-라이브 정렬 — 이미 해소돼 있었음(이 문서의 오류 정정)**: 2026-07-06 착수 시 코드 재확인 결과, 이 항목은 **v51.91/P586에서 이미 완전히 해소**됨 — `screener-backtest-panel`(js/aio-data.js `_aioRenderScreenerBacktest`)이 정확히 이 항목이 요구한 정직 라벨("검증 범위: 가격 파생 4팩터만... 라이브 랭킹에 있는 size/value/quality 팩터와 레짐 적응 가중은 이 검증에 포함되지 않음")을 이미 표시 중이며, 서버(`excludedFactors`/`weightRegime`/`excludedFactorsReason` 필드, `COMP_W` 가중 단일소스화)와 IC 시계열 누적(`backtest-history.json`)까지 포함해 이 항목의 제안보다 더 완전하게 처리돼 있었다. **이 문서(07-02 진단 계승분)가 07-02 시점 정보를 07-06 작성 시 재검증 없이 그대로 옮긴 것이 원인** — CODE-MAP의 "C1 미해결" 오기재(Phase 0-4에서 정정)와 동일한 종류의 실수를 진단자 자신이 반복한 사례. 조치 불필요, 기록만 정정.
10. **[C3] 착수 금지 확인만 — 유지**: score-backtest-history.json 누적 대기(매 사이클 자동 적재 중). forward-5d n≥30 도달 시 재평가 — 그 전에 가중/경계 재튜닝 금지(P599). 이번에도 착수하지 않음(설계상 의도).
11. ✅ **[D-VCP] 파리티 감시 — 완료(2026-07-06, P631)**: `ci-data-pipeline-contract-check.mjs`에 서버/클라 VCP 핵심 파라미터(스윙 N=4 선언·베이스윈도우 min(65,n-10)·52주 캡 min(252,n)·Stage2 -30% 하한·수축깊이 1~45%) 7개 텍스트 계약 추가(RSI parity 체크와 동일 `extractFunctionSource` 패턴 재사용). **주의**: 최초 구현은 느슨한 `\bN=4\b` 정규식이 근처 한국어 주석("좌우 N=4봉 기준")과 실제 코드 선언을 구분 못해 코드만 변경한 뮤테이션을 놓쳤음(직접 뮤테이션 테스트로 발견) — 선언 앵커를 포함한 정밀 패턴으로 재작성 후 baseline 통과 + 뮤테이션 검출 둘 다 실측 확인. 현재 서버/클라 완전 동일(드리프트 0), 이 게이트는 향후 드리프트 방지용.

### Phase 4 — 만성 클래스 (장기, 각각 별도 계획 후 — 이번 세션 미착수)

> 아래 4개 항목은 로드맵 작성 시점(§6 서두)부터 "각각 별도 계획 수립 후" 진행하도록 의도적으로 스코프 밖에 둔 항목들이다. 2026-07-06 세션에서 Phase 0~3(§6 항목 1~11)을 전부 처리했으나, 이 Phase 4는 그 설계 의도대로 미착수 상태를 유지한다 — 각 항목이 독립적인 스코프 판단(CHANGELOG 분할 범위, !important 상한선 값, 전역 상태 이전의 파급 범위, Yahoo 폴백 확장의 검증 계획)을 필요로 하며, 다음 세션에서 사용자와 우선순위를 정한 뒤 개별 착수를 권장한다.
