...
**남은 항목(다음 세션)**: (1) `breadth-stage-summary`/`mtf-verdict-text` 두 표면에 무엇을 채울지 제품 결정(P746) — QA-CHECKLIST 열린 백로그. (2) RM-03 item 3의 실제 구현은 ARX-11(W4/W5/W7 선행) 스코프로 확정. (3) `ci-domain-parity-check.mjs`의 나머지 6개 smoke-only 모델(market/macro/portfolio/screener-ranking/technical) — Fable 자문: screener-ranking은 추출 시 C2(클라 7팩터 vs 서버 4팩터) 불일치가 즉시 표면화되므로 착수 전 어느 산식이 정본인지 제품 결정 선행 필요·CODE-MAP의 `_aioComputeFactorRanks` 좌표 자체가 자기모순이라 재grep 필수(`_context/CODE-MAP.md:113/332/337-338` vs 실측 16020행); market/macro는 대응하는 단일 legacy 산식이 아예 없을 수 있어 "실 parity화"가 아니라 R352에 따른 "toy 퇴역"이 정답일 수 있음. signal은 ARX-11로 별도 확정. (4) ARX-04: 나머지 6개 domain(sentiment는 라이브 렌더 회귀 위험 — market-snapshot.json에 VIX는 있으나 VIX9D/VIX6M 결측·신선도 강등·provenance 3원천 분열로 보류 유지, Fable 재확인; market/themes는 quote 파이어호스 의존; news는 fetch 관점에서는 아직 미착수(오늘 한 것은 계산 로직 추출이지 provider fetch 전환이 아님, `data.json.news` 단일 아티팩트가 있어 오히려 저위험일 수 있다는 Fable 지적 — 다음 후보); portfolio/analysis는 독립 fetch 대상 아님)의 실 fetch 전환, screener·entity의 in-flight fetch abort-on-dispose 보강. (5) ARX-04 완료 판정은 "첫 slice의 legacy fetch/DOM writer 실제 삭제"가 있어야 하며, screener·entity 둘 다 추가식이라 그 기준 미충족 — legacy cutover 시점은 해당 route의 진짜 렌더 전환(W5 ARX-09/10) 때가 자연스러움.
