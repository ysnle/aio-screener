...
> **실물 대조 완료(2026-07-09 2차)**: 20페이지 전체의 실제 섹션 라벨·요소 ID를 스크립트로 전수 추출해 아래 "how"가 가리키는 카드/섹션명이 **전부 실존함을 확인**하고 불일치 5건(signal 가중치, breadth 5/20/50 구성, portfolio VaR→샤프·베타·MDD, kr-home 미국지수 부재, kr-supply 환율카드 부재)을 정정했다. 구현 시 섹션명이 바뀌어 있으면 해당 페이지 DOM을 재확인할 것.
...
- how:
  ...
  - **'엔캐리 언와인드 위험도' 게이지**가 위 원리(엔·금리차·VIX·HYG 합성)를 점수화한 것입니다 — 엔 급등이 보이면 이 게이지부터 확인하세요.
...
4. **QA-CHECKLIST P654-Q 블록**:
   ...
   - P654-Q3: 레지스트리 문안에 R291 위반이 없다. 기계 검사 가능한 부분: 연도/날짜 패턴(`20\d\d년?`, `\d+월 \d+일`) 0건, "현재/지금 ~이다·입니다" 형태의 시장 상태 단정문 0건. 주의: '지금'·'현재' 단어 자체는 금지가 아니다 — "지금 얼마나 공격적이어도 되나"(불변 프레임), "경기 사이클 — 지금 어디?"(실존 섹션명 인용)는 허용. 금지는 특정 시점의 시장 레벨·국면을 사실로 서술하는 문장이다. 기계 검사 + 문안 diff 육안 검수 병행.
