...
- **현재 버전**: v50.41
- **v50.41 선순환 연결 계층 — 크로스-페이지 뉴스 통합**: 단일 뉴스캐시(RSS+텔레그램 6채널·classifyTopic)→분석 8페이지에 토픽 필터 연결. `AIO_NEWS_SURFACE_CONTRACTS`+8페이지(topics macro/geo/semi/earnings/energy), `buildNewsSurfaceModel` 범용 topics 필터, `_aioRenderPageNewsStrip`(insight-box 직후 스트립·pageShown 배선), `AIO.getConnectiveLayerAudit`(8/8·11 surface, 라이브 ok). 트랙2: HOME 신선 텔레그램(6/18 FOMC·SpaceX $175·NVDA Vera China·Nokia AI). 트랙3: `_getV48IntegratedContext` 최상단 현재 레짐(2026-06) 헤더 prepend(4월 이란전쟁 §70/§71 historical 강등). T814. 보류: KR 정적·jensen. R1 7곳+캐시버스터 5곳.
- **v50.40 데이터품질 핫스팟 + 텔레그램/채팅 맥락 신선화**: 앱 audit 진단(자동 최신화 골격 건강·텔레그램 6채널 18건 동적). 트랙B: fetch-data.mjs SYMBOLS에 GBPUSD/CNY/AUDUSD 추가(fxbond "—" 해소) + AIO_DATA_TRUTH_GATE ^KS11 sanity 6000→12000(KOSPI 7,800 정상값 truth-block한 config버그 시정·home 해소) + signal breadth-consensus-verdict lineage. 트랙A: AI 채팅 시장맥락 de-stale(유가 $100+/호르무즈→이란 휴전·유가 $85·메모리 슈퍼사이클). 보류(차기): 크로스-페이지 뉴스 strip(7페이지)·_getV48 텔레그램·KR정적·jensen. R1 7곳+캐시버스터 5곳.
- **v50.39 텔레그램 3채널 통합 + /data-refresh**: 텔레그램(aether/insidertracking/bornlupin) 7일: 6/11 반등 랠리(S&P 7,394 +1.75%/Nasdaq 25,810/Dow 50,841/VIX 19.4) + US-이란 MOU 휴전임박(호르무즈 재개·WTI $84.92 -3.2%, v50.15 유가급등서 반전) + SpaceX IPO($135/$1.78T) + 메모리 슈퍼사이클(DDR5 H1'28·SK하이닉스 +230%/나스닥상장·키옥시아 日1위·HDD랠리·Q1장비 $36.55B) + NVDA AA/AMD $575/Meta $835/나스닥100 NBIS·RKLB·ALAB·CRWV·TER. data-refresh: DATA_SNAPSHOT 본체+미러(R184) 지수/VIX/유가/금/원달러1523/BTC61303/10Y4.47/KOSPI7764 + 내러티브 + HOME뉴스 5건 재큐레이션. 차트는 v50.38 _fetchYahooChartData 복구로 라이브 전환. 미확보 필드 추측금지 SKIPPED. v50.36~38 단일 배포. R1 7곳+캐시버스터 5곳.
- **v50.38 AI 채팅 정성·도메인 데이터 + 초보자 시각 차트**: 조사 — 정성은 기업 깊음·매크로/외환/채권/테마는 도메인 라이브 미주입. 시각=스파크라인뿐, 의존 `_fetchYahooChartData` 정의 부재(8곳 silent skip)로 degraded(사용자 의심 적중). 트랙1(1a) `_fetchYahooChartData` 복구(Yahoo v8×fetchViaProxy)→스파크라인/OHLCV폴백/VIX·HY 정상화. (1b) `_aioLastTechSnap` stash→초보자 "차트 읽기" 카드(추세/위치/모멘텀/한 줄 결론)+`_aioShowTechnicalChart`(technical 심층차트 자동로드). 트랙2 `_fetchDomainContextForChat`: macro/fxbond/themes 도메인 라이브 주입(compute/스냅샷 재사용). 트랙3 기술 게이트 전 컨텍스트 확장. T810~T813. v50.36+v50.37 단일 배포. R1 7곳+캐시버스터 5곳.
- **v50.37 AI 채팅 차별화 업그레이드**: 조사 결론 — 채팅은 이미 22개+ 소스·8모드·검증으로 일반 LLM 대비 막대히 차별화(백엔드라 안 보임). 3트랙. (1) 자연어 스크리너 질의 `_aioRunScreenerQuery`: COMPARE_SCREEN 의도 감지되나 실행 없던 갭 — 자연어 조건 파싱(섹터·시그널·시총·RSI·지수·등락·테마)→실제 SCREENER_DB×_liveData 필터→top12→Claude 해설(`chatSend` 배선+배지). 일반 LLM 불가, 신규 API 0. (2) `AIO_CHAT_SOURCE_REGISTRY` 22소스 선언 카탈로그+`_aioLastChatSources` 매니페스트+`getChatSourceRegistryAudit`(드리프트 0). DART는 planned 확장점. bespoke 실행부 유지. (3) HARD STOP 3분기(종목+시세미수신만 엄격/스크리너 블록값/일반·교육 유연)로 경직 해소. T807~T809. v50.36 포함 단일 배포. R1 7곳+캐시버스터 5곳.
