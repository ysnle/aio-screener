...
| B9 | CODEX-COMPREHENSIVE-DIAGNOSIS WO-6 완료 게이트 잔여분 ("모든 값"의 전면 provenance 리트로핏) + WO-2/WO-3의 survivorship bias·전체 표본 미검증 | **이건 §1 "진짜 블록"이 아니라 순수 엔지니어링/데이터 규모 문제** — 데이터·시간·운영자 결정 어느 것도 막고 있지 않음(단, survivorship bias 해결에 필요한 point-in-time 지수 구성종목 이력은 무료 소스가 없어 "데이터 없음" 성격도 일부 겹침). Codex 원문 완료 게이트("모든 값에 source/observedAt/...", "survivorship 검사 PASS" 등)를 문자 그대로 만족하려면 (WO-6) 20개 페이지 개별 값 전체 리트로핏, (WO-2/3) 무료로는 구할 수 없는 다년치 breadth/PCR/AAII/HY스프레드 PIT 데이터 + 유료 PIT 지수 구성종목 이력이 필요 — 전부 단일 세션 범위 밖. | 다음 세션에서 §2 방식(단계별 커밋)으로 이어서 진행 가능 — 우선순위: (1) `DATA_SNAPSHOT` 필드별 타임스탬프 구조화, (2) evidence/백테스트 결과를 노출하는 전용 UI 패널(현재 WO-6/WO-2/WO-3 모두 콘솔/AI 컨텍스트·JSON 파일 전용), (3) 유료 PIT 데이터 소스 확보 시 survivorship-bias-free 팩터/스코어 재검증 | **2026-07-10, v52.49~52.51, P664/P665/P666**: WO-6 핵심 슬라이스, WO-2(트레이딩 스코어 매크로 서브포뮬러 10년 검증 — 유의미한 음의 상관 발견), WO-3(팩터 모델 10년/120종목 검증 — lowvol 서브팩터 유의미한 음의 상관 발견, composite는 무의미) 전부 축소 범위로 완료. 상세: 각 P번호의 "범위 밖으로 남긴 것" 참조. |

> 핵심: B1·B2·B3 = **데이터 없음**, B4·B6 = **시간 필요**. 코드로 해결되는 게 아니다. B7(외부 서비스 TradingView 상태 불확실)은 자체 렌더로 대체해 2026-07-05 해소됨(P610), B5(운영자 결정: Worker 재배포)는 2026-07-07 운영자가 실제로 재배포해 해소됨(P638) — 둘 다 더 이상 이 표에 해당 없음. B8(AI 채팅 간헐적 403)은 **운영자 결정도 데이터도 아닌 인프라 제약**(Cloudflare 무료 플랜 리전 미고정 + Anthropic 리전 정책) — 완전 제거는 여전히 불가하지만, 재시도 완화책은 같은 날 구현 완료(v52.44/P659, 채팅+번역+브리핑 3개 함수 전체 적용). B9(WO-6 전면 리트로핏 잔여 + WO-2/3 survivorship bias·전체표본 미검증)는 **엔지니어링/데이터 규모 문제일 뿐 진짜 블록 아님** — 각 핵심 슬라이스는 v52.49~52.51/P664~666에서 완료, 전면 확장은 §2 방식으로 다음 세션 이어서 가능(단 survivorship bias 완전 해결은 유료 PIT 데이터 확보가 전제).
