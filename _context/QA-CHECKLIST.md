...
- [ ] QA-EXHAUST-87: **남은 수렴 대상 3종을 처리한다.** P1126(이스케이프)·P1128(OPEX)로 두 종류를 수렴시켰고, 감사에서 확인된 나머지는 (a) **page-ID 목록 5벌** — `ROUTE_PAGE_IDS`(aio-core), `AIO_CRITICAL_10_PAGE_IDS`(aio-data), `PAGES`(aio-core), `breadcrumbMap`(aio-core), `AIO_PAGE_REFRESH_MAP`(aio-data)가 손으로 각각 관리돼 서로 어긋날 수 있고, (b) **TTL/임계 표 5~6벌** — `FRESHNESS_POLICY`·`T`·`AIO_THRESHOLD_REGISTRY`(이상 aio-core), `REFRESH_SCHEDULE`·`_MANUAL_FIELDTS_MAX_DAYS`(이상 aio-data), (c) **상대시간/KST 헬퍼 5벌** — `getTimeAgo`·`_aioKstShortFromIso`·`_getBriefingWindowKST`(aio-data), `_aioRelativeDate`(aio-chat), `AIO.getKstDateParts`(aio-ui). 셋 다 순수 데이터/함수라 동작 위험은 낮지만 어긋나면 라우팅·신선도·표시가 조용히 틀어진다. verify_by: ci-structural-check + ci-runtime-contract-check (각 목록이 단일 원천을 참조하는지)
