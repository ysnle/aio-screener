...
- [ ] QA-EXHAUST-89: **인라인 블록 A~D(11,014줄)를 같은 방식으로 이관한다.** P1132에서 두 가지가 추가로 확정됐다. (1) **이관 전에 같은 이름의 전역 대입을 확인하라** — 블록 F의 `_fetchYahooChartData` 위임 래퍼는 `aio-data.js`의 `window._fetchYahooChartData = ...` 대입에 항상 덮여 죽어 있었는데, 이관으로 실행 순서가 뒤집혀 승자가 되면서 기본값이 호출자에게 새로 적용됐다(headless T1041이 검출). (2) **게이트 단언이 여러 블록에 걸칠 수 있다** — 5개 게이트 중 2줄은 일부만 재지정해야 했다(VKOSPI, KR theme breadth). 블록 A는 core보다 먼저 실행되어 core가 나중에 읽는 전역을 만들므로 특히 주의한다. verify_by: ci-headless-tests + ci-architecture-browser-check + ci-runtime-contract-check + ci-research-flow-contract-check + ci-proxy-continuity-check
