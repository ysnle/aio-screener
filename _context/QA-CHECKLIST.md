...
- [ ] QA-CRED-25: **`data` 그룹에 남은 2건 — CI가 아직 red이고, 그래서 사이트가 아직 v56이 아니다.** P1160으로 스냅샷 발행(A1)·fail-open 커밋·generated-state 결합은 해소됐고 `data-refresh`와 preflight는 통과한다. 남은 것은 (a) **`history-time`**: `[history-field-time-contract] latest row 2026-09-20 field btc: in-session observation promoted as daily close` — `scripts/ci-history-field-time-contract-check.mjs:85`가 `observedMarketSession ∈ {CURRENT_SESSION, DELAYED_IN_SESSION}`이면 `valueBasis === 'previous-completed-close'`를 요구하는데, 24/7 자산(BTC)의 최신 행이 다른 basis로 발행됐다(P1144와 같은 계열의 재발, 프로듀서 `scripts/fetch-data.mjs`의 history 빌더). (b) **`data-lineage` self-test**: `self-test: stale Korea inherited US holiday grace` — `scripts/ci-data-lineage-audit.mjs:264-266`의 픽스처에서 `^KS11`이 미국 Labor Day 휴장 grace를 상속받았다. 즉 `marketClosedGraceEligible`이 grace를 미국 세션 밖 자산까지 넓게 적용한다. 둘 다 원인이 파일·행 단위로 특정됐고, 데이터 시맨틱을 건드리므로 관측된 실패를 고치는 별도 작업으로 진행한다. verify_by: `node scripts/ci-history-field-time-contract-check.mjs` + `node scripts/ci-data-lineage-audit.mjs` PASS, 그리고 그 뒤 CI `Contracts / data` 통과 → `Deploy GitHub Pages` 실행 → 라이브 사이트가 v56인지 확인
