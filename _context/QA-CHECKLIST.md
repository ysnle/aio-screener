...
- [x] QA-CRED-23: **주말마다 시장 스냅샷 발행이 멈춰 사이트 배포가 막혔고, 그걸 막는 게이트가 실제로는 막지 않았다 (P1160).** 원인은 서로 다른 두 개의 24시간 상한이었다 — `deriveMarketSession`이 휴장 확인된 장소도 24h까지만 인정해(금요일 종가를 일요일에 읽으면 46h) 한국 지수·FX·선물·원자재가 `STALE_UNEXPECTED`가 됐고, `quoteQuality`도 같은 24h로 `CLOSED_CURRENT`를 막았다. 미국 지수만 예외였던 비대칭이 버그였다. `CLOSED_VENUE_MAX_AGE_MS`(4일)로 세 지점을 통일했고, 게이트는 이제 발행 결과(`complete===true`, `QG-01_PASS`, `errors===[]`)를 힌트 3종에 대해 단언한다. 추가로 `refresh-data.yml`의 커밋 조건이 검증 게이트를 보지 않던 fail-open을 고쳐, 게이트가 빨간데도 `85d05825`가 올라가던 경로를 닫았다. verify_by: 판정 완료(P1160) — refresh 재실행 성공 + CI `data` 그룹 통과 + Pages 배포로 라이브 확인.
- [ ] QA-CRED-25: **`data` 그룹에 남은 2건 — CI가 아직 red이고, 그래서 사이트가 아직 v56이 아니다.** P1160으로 스냅샷 발행(A1)·fail-open 커밋·generated-state 결합은 해소됐고 `data-refresh`와 preflight는 통과한다. 남은 것은 (a) **`history-time`**: `latest row 2026-09-20 field btc: in-session observation promoted as daily close`(= QA-CRED-26), (b) **`data-lineage` self-test**: `self-test: stale Korea inherited US holiday grace`(= QA-CRED-27). verify_by: 아래 두 항목이 각각 해소된 뒤 `node scripts/ci-data-refresh-audit.mjs` + `node scripts/ci-history-field-time-contract-check.mjs` + `node scripts/ci-data-lineage-audit.mjs` PASS, 이어서 CI `Contracts / data` 통과 → `Deploy GitHub Pages` 실행 → 라이브 사이트 v56 확인
- [ ] QA-CRED-26: **`ci-history-field-time-contract-check.mjs` 안의 두 규칙이 24/7 자산에서 서로 모순된다 — 결정이 필요해 손대지 않았다.** 실측: 최신 history 행의 `btc`는 `observedAt 2026-09-20T12:01:49Z`(BTC는 24/7이라 in-session), `valueBasis: latest-completed-close`, `observedMarketSession: CURRENT_SESSION`이다. 그런데 같은 게이트가 두 가지를 동시에 요구한다 — (i) 85행: `observedMarketSession ∈ {CURRENT_SESSION, DELAYED_IN_SESSION}`이면 `valueBasis`가 반드시 `previous-completed-close`여야 한다. (ii) 99~107행(P1144): 24/7 연속 시세는 "이전 완료일"이 없다고 보고 **관측값을 기록**하며, 스냅샷의 **live 값**(`quote.value`)과 2% 밴드로 비교한다. 프로듀서(`fetch-data.mjs:1639-1669`)는 (ii)의 주석·의도를 그대로 구현했고, 그래서 (i)에 걸린다. (i)을 만족시키려면 라벨을 `previous-completed-close`로 바꿔야 하는데 그러면 live 스냅샷 값에 "이전 종가"라는 거짓 라벨이 붙고, 값을 진짜 이전 종가로 바꾸면 (ii)의 밴드 비교가 운에 좌우된다. **어느 쪽이 옳은지는 데이터 시맨틱 결정이다** — 선택지는 (A) 24/7 자산을 (i)에서 제외(P1144가 같은 파일에서 이미 내린 결정과 정합) 또는 (B) 프로듀서가 UTC 이전 완료일 종가를 쓰고 (ii)를 그에 맞게 조정. 게이트를 통과시키려고 규칙을 느슨하게 만드는 것은 이 저장소가 금지하는 방향이므로, 결정 전에는 적용하지 않았다. verify_by: `node scripts/ci-history-field-time-contract-check.mjs` PASS + 선택한 쪽의 근거를 R로 승격, 그 뒤 CI `Contracts / data` 통과
- [ ] QA-CRED-27: **`data-lineage`의 `marketClosedGraceEligible`이 미국 휴장 grace를 한국 자산까지 넓게 적용한다.** 자체 self-test가 `^KS11`(관측 2026-09-04T06:30Z)에게 미국 Labor Day(2026-09-07) grace를 주는 것을 "stale Korea inherited US holiday grace"로 검출해 게이트가 실패한다(`ci-data-lineage-audit.mjs:264-266`). 즉 grace 판정에 자산의 장소(venue/session) 범위가 걸려 있지 않다. verify_by: `node scripts/ci-data-lineage-audit.mjs` PASS(자체 self-test 포함), grace를 미국 세션 자산으로 한정한 뒤에도 기존 픽스처(주말·휴장 grace가 정당한 경우)가 계속 통과하는지 함께 확인
