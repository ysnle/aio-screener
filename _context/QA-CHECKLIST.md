...
- [ ] QA-EXHAUST-89: **인라인 블록 A~D·F·G(15,128줄)를 같은 방식으로 이관한다.** P1130에서 방법과 블록당 비용을 확정했다 — 새 파일을 만들지 않고 기존 등록 파일에 접어넣으면 매니페스트 4곳·스크립트 맵 2곳이 불변이고, 블록당 필요한 것은 (a) 추출 (b) 그 블록의 텍스트를 단언하는 게이트 소유자 수정 (c) 래칫 기록뿐이다. 다만 A~D·F·G는 **파싱 시점 실행 순서에 의존**할 수 있어(블록 A는 core보다 먼저 실행되어 core가 나중에 읽는 전역을 만들 수 있다) 블록별로 "이 블록이 정의하는 심볼을 파싱 시점에 읽는 곳이 있는가"를 먼저 확인해야 한다. 블록 E는 그 의존이 없어 안전했다. 권장 순서: D(3,750) → C(3,475) → F(3,108) → A(2,611) → B(1,176) → G(1,008). verify_by: ci-architecture-browser-check + ci-headless-tests (이관 후 라우트 20개 sink 값과 0 browser error)
