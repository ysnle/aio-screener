...
- [ ] QA-CRED-05: **공유 Worker만 설정된 상태에서 사용자가 FRED 키를 넣었을 때의 실브라우저 동작은 정적 근거뿐이다.** 릴레이 2순위 배선과 `PRIVATE_ROUTE_REQUIRED` 사유 힌트는 코드 경로로만 확인했다. verify_by: `ci-architecture-browser-check.mjs`(매크로 라우트에서 FRED 계열 값의 출처 라벨 확인)
...
- [ ] QA-CRED-15: **커밋 단계가 `git add` 실패를 삼킬 수 있다 (의도적으로 미수정).** `refresh-data.yml`·`refresh-screener.yml`의 커밋 스텝은 `set -e` 없이 23~25줄의 `[ -f X ] && git add X || true`를 쓰므로, 파일이 **존재하는데** `git add`가 실패해도 마지막 명령이 성공해 스텝이 초록으로 끝난다. 검증된 패치: 스텝 상단에 `set -euo pipefail`을 넣고 각 줄을 `if [ -e "$path" ]; then git add "$path"; fi`로 바꾸거나, `stage(){ [ -e "$1" ] || return 0; git add "$1" || echo "::error::failed to stage $1"; }` + 실패 누적 후 `exit 1`. **적용하지 않은 이유**: 데이터 봇의 프로덕션 스텝을 로컬에서 한 번도 실행해볼 수 없고, 잘못하면 "조용한 결손"이 "초록이던 봇이 멈춤"으로 바뀐다 — 지금 파이프라인은 이미 신선도 실패 상태라 변동을 더하지 않는다. verify_by: 위 패치 적용 후 **지도 하에** `workflow_dispatch` 1회 실행으로 커밋 스텝이 정상 사이클에서 초록인지 확인
