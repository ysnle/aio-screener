...
- [ ] QA-EXHAUST-89: **인라인 블록 A~B와 소형 4개(3,324줄)를 같은 방식으로 이관한다.** 남은 것은 A(1,994 — 포트폴리오·백테스트 랩·워치리스트·채팅 이력·`showToast`/`_escHtmlSafe`/`PF_JOURNAL_KEY`/`CHAT_*`·`getWatchlists` 계열), B(1,176 — 매크로/기술 렌더: `computeMarketHealth`·`updateTechIndicators`·`loadTechCandleChart`·`renderEconCalendar`·`renderYieldCurve`·`updateMacroRegimePill`), 그리고 소형 4개(5,998 33줄 · 7,806 22줄 · 8,874 7줄 · 13,017 92줄 = 154줄)다. 등록은 P1134에서 확정된 **7곳 + CODE-MAP 행**(프리캐시 `sw.js`는 제외 — R621). **블록 A는 core보다 먼저 실행되어 core·B·C가 나중에 읽는 전역을 만든다** — defer로 옮기면 뒤따르는 인라인 블록이 A보다 먼저 실행되어 A의 함수(`showToast` 등, 클래식 스크립트의 전역 함수 선언)를 못 본다. A는 **현재 위치 그대로 비-defer 외부 스크립트**로 두거나(파싱 시점 실행 순서 보존), core·B·C가 쓰는 이름을 먼저 목록화해 지연 호출로 바꿔야 한다. verify_by: ci-headless-tests + ci-architecture-browser-check + ci-runtime-contract-check + ci-decomp-hotspot-check
