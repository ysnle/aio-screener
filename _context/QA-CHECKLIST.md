...
- [ ] QA-EXHAUST-92: **디퍼 순서 변경이 전역 승자를 바꾸는지 사전 점검하는 도구를 만든다.** P1132(래퍼가 승자가 됨)·P1133(파일이 늘어 로드 순서가 길어짐)로 이 위험이 두 번 드러났다. `js/*.js`의 최상위 `function X`와 `window.X =`를 이름별로 모아 "선언과 대입이 둘 이상인 이름" 목록을 출력하고, 그 목록이 비어 있지 않으면 이관 시 수동 확인을 요구하는 검사가 필요하다(QA-EXHAUST-91과 같은 계열). verify_by: ci-structural-check (이중 정의 이름 목록이 0이거나 명시 허용목록)
- [ ] QA-EXHAUST-91: **인라인↔디퍼 이동 시 전역 승자 변화를 기계적으로 탐지한다.** P1132의 회귀(래퍼가 승자가 됨)는 headless가 잡았지만, 그 검사(T1041)는 우연히 그 이름을 검사하고 있었기 때문이다. 일반 규칙으로 만들려면 "전역 이름 N에 대해 선언과 대입이 둘 이상 존재하면 그 목록을 보고한다"는 검사가 필요하다 — `ci-structural-check`의 R280 스캔을 선언뿐 아니라 **대입**까지 확장하는 방향. verify_by: ci-structural-check (선언+대입 이중 정의 목록이 0 또는 명시 허용목록)
