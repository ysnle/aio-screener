...
- [ ] QA-EXHAUST-89: **인라인 블록 A(1,994)와 소형 인라인 4개(154줄)를 이관한다.** A는 포트폴리오·백테스트 랩·워치리스트·채팅 이력(`showToast`/`_escHtmlSafe`/`PF_JOURNAL_KEY`/`WL_*`/`CHAT_DEFAULT_CHIPS`/`CHAT_HISTORY_LS`/`getWatchlists` 계열). 소형은 5,998(33줄)·7,806(22줄)·8,874(7줄)·13,017(92줄). 등록은 **7곳 + CODE-MAP 행**이며 프리캐시(`sw.js`)는 제외한다(R621). **A는 여전히 core보다 먼저 실행되어 core·B·C가 읽는 전역을 만든다** — R622에 따라 A의 태그는 **`defer` 그룹 맨 앞**(현재 macro-tech/kr-data/pages보다도 앞, 즉 첫 번째 defer)에 두어 "모든 다른 런타임보다 먼저" 성질을 보존한다. 소형 4개도 각자의 원래 위치 순서를 유지한다. 순서를 건드린 뒤에는 **반드시 `ci-architecture-browser-check`(실브라우저)를 돌린다** — P1135에서 headless 1,133건이 전부 통과하는데도 ui 파일이 통째로 죽었다. verify_by: ci-architecture-browser-check + ci-headless-tests + ci-decomp-hotspot-check + ci-structural-check
