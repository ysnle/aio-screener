...
- [ ] QA-EXHAUST-89: **인라인 블록 A~D·F(14,122줄)를 같은 방식으로 이관한다.** 방법은 P1130에서, **비용 구조는 P1131에서 정정**했다 — 블록당 비용은 일정하지 않고 **그 블록의 텍스트를 게이트가 얼마나 단언하는가에 비례**한다. 실측: 블록 E는 게이트 1곳, 블록 G는 **15곳**(4개 게이트, 그중 5곳은 "일부만" 재지정). 정적 grep으로는 셀 수 없다 — 단언이 심볼명이 아니라 인접 임의 텍스트(`extractChips(visible)`, `query: q`, `'home': 'home'` 등)를 검사하고, 한 줄 `check()`가 여러 파일을 섞기 때문이다. 따라서 **사전 추정 대신 "이동 → 게이트 실행 → 실패한 단언의 소유자만 재지정" 경험적 루프**를 쓴다(정의는 새 파일, 호출·마크업은 셸). 파싱 시점 의존도 블록별 확인이 필요하다(블록 A는 core보다 먼저 실행되어 core가 나중에 읽는 전역을 만든다). 권장 순서: D(3,750) → C(3,475) → F(3,108) → A(2,611) → B(1,176). verify_by: ci-architecture-browser-check + ci-headless-tests + ci-runtime-contract-check (이관 후 20 라우트 sink 값, 0 browser error, 런타임 계약 통과)
