...
- [ ] QA-EXHAUST-89: **인라인 블록 A~C(6,644줄)를 같은 방식으로 이관한다.** 남은 블록은 A(1,993 — 사이드바 설정·포트폴리오·워치리스트·채팅 이력·라이브 데이터 헬퍼·채팅 컨텍스트 빌더), B(1,176 — 매크로/기술 렌더), C(3,475 — KR 데이터 플레인)다. P1133에서 **새 파일 등록 8곳 체크리스트**가 확정됐으므로 C는 `js/aio-kr-data.js` 같은 전용 파일로 보내는 것이 맞다(`aio-data.js`에 접어넣으면 20k가 되어 측면 이동이 된다). **블록 A는 core보다 먼저 실행되어 core가 나중에 읽는 전역을 만들므로**, 이관 시 core의 모듈 평가 시점에 그 전역이 없어질 수 있다 — A는 defer가 아닌 **비-defer 스크립트로 core 앞에** 두거나 core 쪽 참조를 지연 호출로 바꿔야 한다. verify_by: ci-headless-tests + ci-architecture-browser-check + ci-runtime-contract-check + ci-data-pipeline-contract-check + ci-decomp-hotspot-check
