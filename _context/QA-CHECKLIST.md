...
- [ ] QA-CRED-31: **`ci-user-journey-quality-check`의 `route shell macro`만 남았다 — 사이트 배포를 막는 마지막 게이트.** 그 하네스가 `window.showPage('macro', null)` 후 350ms에 읽으면 `moduleState: 'failed'`, `controls: []`, 정적 제목 `거시경제`는 렌더됨, 오류 문구는 fail-closed 기본문구다. 즉 macro의 지연 모듈 로드/마운트가 실패한다 — 로더는 `resolvePage()` 거부 또는 `.then()` 안 마운트 예외에서 'failed'를 쓰므로(`src/app/router.js:67-79`) 둘 중 하나이고, 350ms 대기 문제는 아니다(지연이면 'loading'으로 남는다). 다음 단계: 그 `.catch`가 받은 `error.message`를 임시로 노출해 실제 원인을 확정한 뒤 수정. verify_by: `node scripts/ci-user-journey-quality-check.mjs` PASS → CI `Browser / browser-knowledge` PASS → `Attest` 실행 → `Deploy GitHub Pages` → 라이브 사이트 v56 확인
