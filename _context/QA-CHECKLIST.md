...
- [ ] QA-CRED-20: **메이저 액션 업과 wrangler 통일은 아직 한 번도 실행되지 않았다.** checkout v5 / setup-node v5는 대상 Node 24 강제를 해소하지만 이 저장소 CI에서 실행된 적이 없고, `deploy-data-plane.yml`의 wrangler 4.44→4.120 통일도 첫 실행 전이다. verify_by: 다음 push의 CI(`ci.yml` 전 샤드) + 수동 `Deploy fast data plane` 실행이 PASS 하는지
