...
- [ ] QA-EXHAUST-90: **3단계 — 라우트 ID 20개를 단일 원천으로 통합한다.** 같은 20개 집합이 `ROUTE_PAGE_IDS`(aio-core), `src/app/routes.js ROUTE_IDS`, `PAGES` 키, `AIO_ROUTE_REGISTRY`, `breadcrumbMap`에 **다른 순서로** 중복돼 있고, `CRITICAL_5`/`ANALYSIS_5`/`WORKFLOW_5`/`EDUCATION`은 그 위의 의도적 분할이다. 단순 병합이 아니라 **한 목록에서 나머지를 파생**시키는 작업이며, `architecture/route-owners.json`의 `counts.*`를 계산 순서대로 재생성하고 `getRouteIAAudit`의 DOM 존재 단언과 헤드리스 라우트 계약(T788 등)을 함께 통과시켜야 한다. verify_by: ci-architecture-contract-check + ci-structural-check + ci-headless-tests
